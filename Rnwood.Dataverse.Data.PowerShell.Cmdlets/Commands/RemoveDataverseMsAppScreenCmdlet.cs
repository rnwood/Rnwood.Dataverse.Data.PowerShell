...
namespace Rnwood.Dataverse.Data.PowerShell.Commands
{
    ...
    public class RemoveDataverseMsAppScreenCmdlet : PSCmdlet
    {
        ...
        {
            // First pass: read all entries into memory so we can check Controls/*.json
            var allEntries = new System.Collections.Generic.Dictionary<string, byte[]>(StringComparer.OrdinalIgnoreCase);
            using (var firstPass = new MemoryStream(originalBytes))
            using (var zipIn = new ZipInputStream(firstPass))
            {
                ZipEntry e;
                while ((e = zipIn.GetNextEntry()) != null)
                    allEntries[e.Name.Replace('\\', '/')] = ReadZipEntryBytes(zipIn);
            }

            string srcYamlFile = $"Src/{ScreenName}.pa.yaml";
            bool screenFound = allEntries.ContainsKey(srcYamlFile);

            // Identify any Controls/*.json that has TopParent.Name == ScreenName
            var controlsFilesToRemove = new System.Collections.Generic.HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (var kv in allEntries)
            {
                string key = kv.Key;
                if (!(key.StartsWith("Controls/", StringComparison.OrdinalIgnoreCase) ||
                      key.StartsWith("Components/", StringComparison.OrdinalIgnoreCase)))
                    continue;
                ...
                try
                {
                    ...
                    if (doc.RootElement.TryGetProperty("TopParent", out var tp) &&
                        tp.TryGetProperty("Name", out var nameProp) &&
                        nameProp.GetString() == ScreenName)
                    {
                        controlsFilesToRemove.Add(key);
                        screenFound = true;
                    }
                }
            ...
            // Second pass: write all entries except the ones to remove
            ...
            using (var zipOutputStream = new ZipOutputStream(resultStream))
            {
                ...
                foreach (var kv in allEntries)
                {
                    ...
                    if (entryName == srcYamlFile)
                    {
                        WriteVerbose($"Removing screen YAML '{entryName}'");
                        continue;
                    }
                    if (controlsFilesToRemove.Contains(entryName))
                    {
                        WriteVerbose($"Removing screen controls JSON '{entryName}'");
                        continue;
                    }

                    AddZipEntry(zipOutputStream, entryName, kv.Value);
                }
