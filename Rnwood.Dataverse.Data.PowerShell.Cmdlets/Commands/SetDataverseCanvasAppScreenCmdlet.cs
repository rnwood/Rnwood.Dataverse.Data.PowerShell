...
namespace Rnwood.Dataverse.Data.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "DataverseCanvasAppScreen", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Guid))]
    public class SetDataverseCanvasAppScreenCmdlet : OrganizationServiceCmdlet
    {
        /// <summary>
        /// Gets or sets the Canvas app ID to add/update screen in.
        /// </summary>
        [Parameter(Mandatory = true, HelpMessage = "ID of the Canvas app to add/update screen in")]
        public Guid CanvasAppId { get; set; }

        /// <summary>
        /// Gets or sets the screen name.
        /// </summary>
        [Parameter(Mandatory = true, Position = 0, HelpMessage = "Name of the screen")]
        public string ScreenName { get; set; }

        /// <summary>
        /// Gets or sets the YAML content for the screen.
        /// </summary>
        [Parameter(Mandatory = true, Position = 1, HelpMessage = "YAML content for the screen")]
        public string YamlContent { get; set; }

        /// <summary>
        /// Gets or sets the path to a YAML file containing the screen definition.
        /// </summary>
        [Parameter(HelpMessage = "Path to a YAML file containing the screen definition")]
        public string YamlFilePath { get; set; }

        /// <summary>
        /// Gets or sets whether to skip creating a new screen.
        /// </summary>
        [Parameter(HelpMessage = "If set, skips creating new screens (only updates existing ones)")]
        public SwitchParameter NoCreate { get; set; }

        /// <summary>
        /// Gets or sets whether to skip updating existing screens.
        /// </summary>
        [Parameter(HelpMessage = "If set, skips updating existing screens (only creates new ones)")]
        ...
        protected override void ProcessRecord()
        {
            ...
            string action = $"Set screen '{ScreenName}' in Canvas app '{CanvasAppId}'";
            ...
            // Modify .msapp file to add/update screen
            byte[] modifiedMsappBytes = ModifyMsAppScreen(msappBytes, yaml);
            ...
            WriteVerbose($"Screen '{ScreenName}' set successfully in Canvas app");
                ...
                using (var zipOutputStream = new ZipOutputStream(resultStream))
                {
                    ...
                    while ((entry = zipInputStream.GetNextEntry()) != null)
                    {
                        if (entry.Name == screenFileName)
                        {
                            ...
                            {
                                WriteVerbose($"Screen '{ScreenName}' exists, skipping update (NoUpdate flag set)");
                                // Copy existing entry
                    ...
                    if (!screenExists)
                    {
                        ...
                        {
                            WriteVerbose($"Screen '{ScreenName}' does not exist, skipping creation (NoCreate flag set)");
                        }
                        else
                        {
