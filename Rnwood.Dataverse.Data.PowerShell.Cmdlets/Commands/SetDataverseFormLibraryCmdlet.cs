    ...
    public class SetDataverseFormLibraryCmdlet : OrganizationServiceCmdlet
    {
        ...
        private void ValidateWebResourceType(Entity webResource, string webResourceName)
        {
            if (webResource.Contains("webresourcetype"))
            {
                ...
                {
                    throw new InvalidOperationException($"Web resource '{webResourceName}' is not a JavaScript file. Only JavaScript web resources (webresourcetype=3) can be added as form libraries. Found webresourcetype={webResourceType.Value}.");
                }
            }
