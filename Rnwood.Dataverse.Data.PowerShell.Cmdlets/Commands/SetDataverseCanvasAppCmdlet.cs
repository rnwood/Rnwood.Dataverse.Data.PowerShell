...
namespace Rnwood.Dataverse.Data.PowerShell.Commands
{
    ...
    public class SetDataverseCanvasAppCmdlet : OrganizationServiceCmdlet
    {
        ...
        {
            // Extract database references from the msapp's DataSources.json
            string databaseReferencesJson = BuildDatabaseReferencesJson(msappBytes);
            WriteVerbose($"DatabaseReferences: {databaseReferencesJson}");
            ...
            using (var zipOutputStream = new ZipOutputStream(memoryStream))
            {
                ...
                string customizationsXml = CreateCustomizationsXml(canvasAppName, displayName, databaseReferencesJson);
        ...
        private string CreateCustomizationsXml(string canvasAppName, string displayName, string databaseReferencesJson)
        {
            XNamespace ns = "http://www.w3.org/2001/XMLSchema-instance";
                ...
                new XElement("ImportExportXml",
                    ...
                    new XElement("CanvasApps",
                        new XElement("CanvasApp",
                            ...
                            new XElement("DatabaseReferences", databaseReferencesJson),
        ...
        /// <summary>
        /// Reads DataSources.json from the msapp ZIP and builds the DatabaseReferences JSON
        /// that Dataverse needs in customizations.xml for native CDS data sources to work.
        /// </summary>
        private string BuildDatabaseReferencesJson(byte[] msappBytes)
        {
            try
            {
                using (var msappStream = new MemoryStream(msappBytes))
                using (var archive = new ZipArchive(msappStream, ZipArchiveMode.Read))
                {
                    var dsEntry = archive.GetEntry("References/DataSources.json") ?? archive.GetEntry("DataSources.json");
                    if (dsEntry == null)
                    {
                        WriteVerbose("No DataSources.json found in msapp, using empty DatabaseReferences");
                        return "{}";
                    }

                    string dsJson;
                    using (var stream = dsEntry.Open())
                    using (var reader = new StreamReader(stream))
                    {
                        dsJson = reader.ReadToEnd();
                    }

                    var dsDoc = JsonNode.Parse(dsJson);
                    var dataSources = dsDoc?["DataSources"]?.AsArray();
                    if (dataSources == null || dataSources.Count == 0)
                    {
                        return "{}";
                    }

                    // Group data sources by DatasetName, only include NativeCDSDataSourceInfo types
                    var datasetGroups = new Dictionary<string, List<JsonNode>>();
                    foreach (var ds in dataSources)
                    {
                        var type = ds?["Type"]?.GetValue<string>();
                        var datasetName = ds?["DatasetName"]?.GetValue<string>();

                        if (type == "NativeCDSDataSourceInfo" && !string.IsNullOrEmpty(datasetName))
                        {
                            if (!datasetGroups.ContainsKey(datasetName))
                            {
                                datasetGroups[datasetName] = new List<JsonNode>();
                            }
                            datasetGroups[datasetName].Add(ds);
                        }
                    }

                    if (datasetGroups.Count == 0)
                    {
                        return "{}";
                    }

                    // Build the DatabaseReferences JSON
                    var dbRefs = new JsonObject();
                    foreach (var kvp in datasetGroups)
                    {
                        var datasetName = kvp.Key;
                        var sources = kvp.Value;

                        var dataSourcesObj = new JsonObject();
                        foreach (var ds in sources)
                        {
                            var name = ds?["Name"]?.GetValue<string>();
                            var entitySetName = ds?["EntitySetName"]?.GetValue<string>();
                            var logicalName = ds?["LogicalName"]?.GetValue<string>();

                            if (!string.IsNullOrEmpty(name) && !string.IsNullOrEmpty(entitySetName))
                            {
                                var entry = new JsonObject
                                {
                                    ["entitySetName"] = entitySetName,
                                    ["logicalName"] = logicalName ?? entitySetName
                                };
                                dataSourcesObj[name] = entry;
                            }
                        }

                        var datasetObj = new JsonObject
                        {
                            ["databaseDetails"] = new JsonObject
                            {
                                ["referenceType"] = "Environmental",
                                ["environmentName"] = datasetName,
                                ["overrideValues"] = new JsonObject
                                {
                                    ["status"] = "NotSpecified"
                                }
                            },
                        ...
                        dbRefs[datasetName] = datasetObj;
                    }

                    return dbRefs.ToJsonString(new JsonSerializerOptions { WriteIndented = false });
                }
            }
            catch (Exception ex)
            {
                WriteWarning($"Failed to extract DatabaseReferences from msapp: {ex.Message}. Using empty references.");
                return "{}";
            }
        }
