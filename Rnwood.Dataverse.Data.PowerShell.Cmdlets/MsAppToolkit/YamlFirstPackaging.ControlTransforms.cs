...
{    private static string ResolveComponentTemplateName(string componentName, JsonObject template, Dictionary<string, JsonObject> metadataByName)
        ...
        IReadOnlyList<Dictionary<string, byte[]>>? referenceEntriesList = null)
    {
        ...
        // Build a reference entries list if not provided (for backward compatibility with callers
        // that don't pass one, e.g. non-pack code paths).
        var refList = referenceEntriesList;
        if (refList is null)
        {
            ReadEmbeddedAllControlsEntries(out var embeddedEntries);
            refList = new[] { embeddedEntries };
        }

        foreach (var refEntries in refList)
        {
            ...
            RegisterPcfCatalogEntries(ReadPcfTemplates(refEntries));
        }
    ...
    private static void ApplyGalleryTemplateTransform(JsonObject galleryNode, GalleryTemplateDescriptor? descriptor)
    {
        ...
        if (galleryTemplateChild is null)
        {
            ...
            var preservedTemplateNode = galleryNode["__PreservedGalleryTemplateNode"] as JsonObject;
            ...
            if (preservedTemplateNode is not null)
            {
                galleryTemplateChild = (JsonObject)preservedTemplateNode.DeepClone();
                if (string.IsNullOrWhiteSpace(galleryTemplateChild["Name"]?.GetValue<string>() ?? string.Empty))
                {
                    galleryTemplateChild["Name"] = string.IsNullOrWhiteSpace(preservedTemplateName)
            ...
            // galleryTemplate is always auto-generated by the gallery runtime.
            galleryTemplateChild["IsFromScreenLayout"] = true;
            galleryTemplateChild["IsAutoGenerated"] = true;
        ...
        // Preserve source PublishOrderIndex values for fidelity; only assign missing entries.
        var maxPublishOrder = 0;
        foreach (var child in children.OfType<JsonObject>())
        {
            if ((child["PublishOrderIndex"] as JsonValue)?.TryGetValue<int>(out var existingOrder) == true)
            {
                maxPublishOrder = Math.Max(maxPublishOrder, existingOrder);
            }
        }
        ...
        foreach (var child in children.OfType<JsonObject>())
        {
            ...
            {
                maxPublishOrder++;
                child["PublishOrderIndex"] = maxPublishOrder;
            }
        }
        ...
        galleryNode.Remove("__PreservedGalleryTemplateNode");
    }
        ...
        IReadOnlyDictionary<string, JsonObject> topComponentFiles)
    {
        ...
        if (root["ControlCount"] is JsonObject existingControlCount
            && existingControlCount.Count == 0)
        {
            // Preserve source fidelity: some exported apps intentionally keep an empty ControlCount object.
            return;
        }
        ...
        Dictionary<string, JsonObject> topComponentFiles)
    {
        ...
        static void PromoteToAutoLayoutChild(JsonObject node)
        {
            ...
            foreach (var rule in existingRules.OfType<JsonObject>())
            {
                ...
                // or create a new entry if the baseline didn't seed one.
                var script = rule["InvariantScript"]?.GetValue<string>() ?? string.Empty;
                ...
                {
                    var category = rule["Category"]?.GetValue<string>() ?? "Design";
                    var newEntry = new JsonObject
                    {
                        ...
                        ["ControlPropertyState"] = property,
                    };
                    dynamicPropsArray.Add(newEntry);
                }
            ...
            // Remove AutoLayout properties from ControlPropertyState – they are now
            // exclusively owned by DynamicProperties and must not appear in CPS.
            if (node["ControlPropertyState"] is JsonArray cpsArray)
            {
                var filtered = new JsonArray();
                foreach (var entry in cpsArray)
                {
                    var propName = GetStatePropertyName(entry);
                    if (!AutoLayoutLayoutPropertyNames.Contains(propName))
                    {
                        filtered.Add(entry!.DeepClone());
                    }
                }

                node["ControlPropertyState"] = filtered;
            }
        }
