...
public sealed class MsAppDocument
{
    ...
    public static MsAppDocument Load(byte[] msappBytes)
    {
        var entries = new Dictionary<string, byte[]>(StringComparer.OrdinalIgnoreCase);

        using (var memStream = new MemoryStream(msappBytes))
    ...
    private static MsAppDocument LoadFromEntries(Dictionary<string, byte[]> entries)
    {
    ...
    public string ExportScreenYaml(string screenName, bool includeHeader = true, bool includeDefaults = false)
    {
        ...
        // Prefer the YAML source from Src/ if it exists.  This preserves the compact
        // representation (only non-default properties) written by Studio or by
        // ImportScreenYaml.  Fall back to generating from Controls JSON only when
        // no source YAML is available (e.g. newly-created screens).
        var srcKey = $"Src/{screen.Name}.pa.yaml";
        if (_entries.TryGetValue(srcKey, out var srcBytes) && srcBytes.Length > 0)
        {
            var srcYaml = Encoding.UTF8.GetString(srcBytes);

            if (includeHeader)
            {
                // If the source already contains the header, return as-is.
                if (srcYaml.TrimStart().StartsWith("#"))
                {
                    return srcYaml;
                }

                return string.Join(Environment.NewLine, HeaderLines) + Environment.NewLine + srcYaml;
            }
            else
            {
                // Strip header comment lines if present.
                var lines = srcYaml.Split(new[] { "\r\n", "\n" }, StringSplitOptions.None);
                var startIndex = 0;
                while (startIndex < lines.Length && lines[startIndex].TrimStart().StartsWith("#"))
                {
                    startIndex++;
                }
    ...
    private static object ToYamlScreenPayload(ControlNode screen, bool includeDefaults)
    {
        ...
        if (!string.IsNullOrWhiteSpace(screen.VariantName))
        {
            payload["Variant"] = screen.VariantName;
        }

        // Export MetadataKey for the screen node so it round-trips through YAML.
        // IsFromScreenLayout and IsAutoGenerated are intentionally omitted: they are
        // not in the pa.schema.yaml for Screen-instance (only Properties/Children are).
        // Those flags are inferred during pack from the MetaDataIDKey pattern.
        // Strip %...ID% wrapper to match Studio convention (e.g. "%Form.ID%" → "Form").
        var screenMetadataKey = screen.Json["MetaDataIDKey"]?.GetValue<string>() ?? string.Empty;
        if (!string.IsNullOrWhiteSpace(screenMetadataKey))
        {
            if (screenMetadataKey.StartsWith("%") && screenMetadataKey.EndsWith(".ID%"))
            {
                screenMetadataKey = screenMetadataKey.Substring(1, screenMetadataKey.Length - 5);
            }
            payload["MetadataKey"] = screenMetadataKey;
        }
    ...
    private static object ToYamlControlPayload(ControlNode node, bool includeDefaults)
    {
        ...
        // Export structural fields that Studio uses for data cards and form controls.
        var isLocked = (node.Json["IsLocked"] as System.Text.Json.Nodes.JsonValue)?.GetValue<bool>() ?? false;
        if (isLocked)
        {
            payload["IsLocked"] = true;
        }

        // The JSON field is "LayoutName" (e.g. "vertical" for Form controls).
        var layout = node.Json["LayoutName"]?.GetValue<string>() ?? string.Empty;
        if (!string.IsNullOrWhiteSpace(layout))
        {
            payload["Layout"] = layout;
        }

        // The JSON field is "MetaDataIDKey" (e.g. "%name.ID%" for data cards, "%Form.ID%" for forms).
        // Studio-generated YAML strips the %...ID% wrapper for readability, so we do the same.
        var metadataKey = node.Json["MetaDataIDKey"]?.GetValue<string>() ?? string.Empty;
        if (!string.IsNullOrWhiteSpace(metadataKey))
        {
            // Strip %...ID% wrapper to match Studio convention (e.g. "%FieldName.ID%" → "FieldName")
            if (metadataKey.StartsWith("%") && metadataKey.EndsWith(".ID%"))
            {
                metadataKey = metadataKey.Substring(1, metadataKey.Length - 5);
            }
            payload["MetadataKey"] = metadataKey;
        }

        // IsFromScreenLayout and IsAutoGenerated are intentionally omitted from YAML.
        // They are NOT in the pa.schema.yaml for Control-instance (additionalProperties:false).
        // Including them would cause PA Studio schema-validation failures that prevent
        // Children from rendering.  These flags are inferred during pack from the
        // MetaDataIDKey pattern (controls whose MetaDataIDKey starts and ends with '%'
        // are auto-generated layout/form controls).
        ...
        // Include DynamicProperties (e.g. FillPortions, AlignInContainer, LayoutMinHeight,
        // LayoutMinWidth, LayoutMaxHeight, LayoutMaxWidth) so they round-trip through
        // YAML → Rules → FixAutoLayoutChildDynamicProperties.
        // Skip entries whose values match the well-known baseline defaults that
        // EnsureAutoLayoutChildBaseline will recreate during pack.
        if (node.Json["DynamicProperties"] is System.Text.Json.Nodes.JsonArray dynProps)
        {
            foreach (var dynEntry in dynProps.OfType<System.Text.Json.Nodes.JsonObject>())
            {
                var propName = dynEntry["PropertyName"]?.GetValue<string>() ?? string.Empty;
                if (string.IsNullOrWhiteSpace(propName) || properties.ContainsKey(propName))
                {
                    continue;
                }

                var script = (dynEntry["Rule"] as System.Text.Json.Nodes.JsonObject)?["InvariantScript"]?.GetValue<string>() ?? string.Empty;

                if (IsAutoLayoutBaselineDefault(propName, script, node.Children.Count > 0))
                {
                    continue;
                }

                properties[propName] = PrefixFormula(script);
            }
        }
    ...
    /// <summary>
    /// Returns true if the DynamicProperty name/value pair matches the well-known baseline
    /// default that <c>EnsureAutoLayoutChildBaseline</c> will recreate during pack.
    /// Container status is inferred from whether the control has children.
    /// </summary>
    private static bool IsAutoLayoutBaselineDefault(
        string propertyName, string script, bool hasChildren)
    {
        // LayoutMaxHeight and LayoutMaxWidth always default to 0 regardless of control type.
        if (string.Equals(propertyName, "LayoutMaxHeight", StringComparison.OrdinalIgnoreCase)
            && string.Equals(script, "0", StringComparison.Ordinal))
        {
            return true;
        }

        if (string.Equals(propertyName, "LayoutMaxWidth", StringComparison.OrdinalIgnoreCase)
            && string.Equals(script, "0", StringComparison.Ordinal))
        {
            return true;
        }
        ...
        if (hasChildren)
        {
            // Container defaults: FillPortions=1, AlignInContainer=Stretch,
            // LayoutMinHeight=32, LayoutMinWidth=32.
            if (string.Equals(propertyName, "FillPortions", StringComparison.OrdinalIgnoreCase)
                && string.Equals(script, "1", StringComparison.Ordinal))
                return true;

            if (string.Equals(propertyName, "AlignInContainer", StringComparison.OrdinalIgnoreCase)
                && string.Equals(script, "AlignInContainer.Stretch", StringComparison.OrdinalIgnoreCase))
                return true;

            if (string.Equals(propertyName, "LayoutMinHeight", StringComparison.OrdinalIgnoreCase)
                && string.Equals(script, "32", StringComparison.Ordinal))
                return true;

            if (string.Equals(propertyName, "LayoutMinWidth", StringComparison.OrdinalIgnoreCase)
                && string.Equals(script, "32", StringComparison.Ordinal))
                return true;
        }
        else
        {
            // Non-container defaults: FillPortions=0, AlignInContainer=SetByContainer.
            if (string.Equals(propertyName, "FillPortions", StringComparison.OrdinalIgnoreCase)
                && string.Equals(script, "0", StringComparison.Ordinal))
                return true;

            if (string.Equals(propertyName, "AlignInContainer", StringComparison.OrdinalIgnoreCase)
                && string.Equals(script, "AlignInContainer.SetByContainer", StringComparison.OrdinalIgnoreCase))
                return true;

            // LayoutMinHeight/Width defaults for non-containers are prototype-dependent
            // (Height/Width rule values) and cannot be safely determined at export time
            // because the packed control may have different rules when the baseline runs.
        }
