...
public static partial class YamlFirstPackaging
{
    ...
    /// <summary>Compact (non-indented) serializer with relaxed escaping so embedded JSON strings use \" instead of \u0022.</summary>
        ...
        string? apiId = null)
    {
        ...
        var booleanAttrPayload = await DataverseGetJsonAsync(normalizedEnvironmentUrl, token,
            $"EntityDefinitions(LogicalName='{escapedTable}')/Attributes/Microsoft.Dynamics.CRM.BooleanAttributeMetadata?$select=MetadataId,LogicalName,SchemaName,DisplayName&$expand=OptionSet");
        var picklistAttrPayload = await DataverseGetJsonAsync(normalizedEnvironmentUrl, token,
            $"EntityDefinitions(LogicalName='{escapedTable}')/Attributes/Microsoft.Dynamics.CRM.PicklistAttributeMetadata?$select=MetadataId,LogicalName,SchemaName,DisplayName&$expand=OptionSet");
        var multiPicklistAttrPayload = await DataverseGetJsonAsync(normalizedEnvironmentUrl, token,
            $"EntityDefinitions(LogicalName='{escapedTable}')/Attributes/Microsoft.Dynamics.CRM.MultiSelectPicklistAttributeMetadata?$select=MetadataId,LogicalName,SchemaName,DisplayName&$expand=OptionSet");
        var stateAttrPayload = await DataverseGetJsonAsync(normalizedEnvironmentUrl, token,
            $"EntityDefinitions(LogicalName='{escapedTable}')/Attributes/Microsoft.Dynamics.CRM.StateAttributeMetadata?$select=MetadataId,LogicalName,SchemaName,DisplayName&$expand=OptionSet");
        var statusAttrPayload = await DataverseGetJsonAsync(normalizedEnvironmentUrl, token,
            $"EntityDefinitions(LogicalName='{escapedTable}')/Attributes/Microsoft.Dynamics.CRM.StatusAttributeMetadata?$select=MetadataId,LogicalName,SchemaName,DisplayName&$expand=OptionSet");
        ...
        NormalizeEntityMetadataContextUrl(tableEntityMetadata, normalizedEnvironmentUrl);

        var iconUrl = BuildIconUrl(normalizedEnvironmentUrl, entityMetadata);

        var tableDefinitionObj = new JsonObject
        {
            ["TableName"] = logicalName,
            ["EntityMetadata"] = tableEntityMetadata.ToJsonString(JsonCompactOptions),
            ["PicklistOptionSetAttribute"] = picklistAttrPayload.ToJsonString(JsonCompactOptions),
            ["MultiSelectPicklistOptionSetAttribute"] = multiPicklistAttrPayload.ToJsonString(JsonCompactOptions),
            ["StateOptionSetAttribute"] = stateAttrPayload.ToJsonString(JsonCompactOptions),
            ["StatusOptionSetAttribute"] = statusAttrPayload.ToJsonString(JsonCompactOptions),
            ["BooleanOptionSetAttribute"] = booleanAttrPayload.ToJsonString(JsonCompactOptions),
            ["EntityNameOptionSetAttribute"] = entityNameAttrPayload.ToJsonString(JsonCompactOptions),
            ["Views"] = viewsPayload.ToJsonString(JsonCompactOptions),
            ["DefaultPublicView"] = defaultPublicViewPayload.ToJsonString(JsonCompactOptions),
            ["IconUrl"] = iconUrl,
            ["Forms"] = formsPayload.ToJsonString(JsonCompactOptions),
        };
        ...
        var nativeEntry = new JsonObject
        {
            ...
            ["TableDefinition"] = JsonValue.Create(tableDefinitionObj.ToJsonString(JsonCompactOptions)),
        ...
        Func<string, Task<string>>? accessTokenProvider = null)
    {
        ...
        var booleanAttrPayload = await DataverseGetJsonAsync(normalizedEnvironmentUrl, accessToken, $"EntityDefinitions(LogicalName='{escapedTable}')/Attributes/Microsoft.Dynamics.CRM.BooleanAttributeMetadata?$select=MetadataId,LogicalName,SchemaName,DisplayName&$expand=OptionSet");
        var picklistAttrPayload = await DataverseGetJsonAsync(normalizedEnvironmentUrl, accessToken, $"EntityDefinitions(LogicalName='{escapedTable}')/Attributes/Microsoft.Dynamics.CRM.PicklistAttributeMetadata?$select=MetadataId,LogicalName,SchemaName,DisplayName&$expand=OptionSet");
        var multiPicklistAttrPayload = await DataverseGetJsonAsync(normalizedEnvironmentUrl, accessToken, $"EntityDefinitions(LogicalName='{escapedTable}')/Attributes/Microsoft.Dynamics.CRM.MultiSelectPicklistAttributeMetadata?$select=MetadataId,LogicalName,SchemaName,DisplayName&$expand=OptionSet");
        var stateAttrPayload = await DataverseGetJsonAsync(normalizedEnvironmentUrl, accessToken, $"EntityDefinitions(LogicalName='{escapedTable}')/Attributes/Microsoft.Dynamics.CRM.StateAttributeMetadata?$select=MetadataId,LogicalName,SchemaName,DisplayName&$expand=OptionSet");
        var statusAttrPayload = await DataverseGetJsonAsync(normalizedEnvironmentUrl, accessToken, $"EntityDefinitions(LogicalName='{escapedTable}')/Attributes/Microsoft.Dynamics.CRM.StatusAttributeMetadata?$select=MetadataId,LogicalName,SchemaName,DisplayName&$expand=OptionSet");
    ...
    private static async Task<JsonObject> DataverseGetJsonAsync(string environmentUrl, string accessToken, string relativePath)
    {
        ...
        var url = relativePath.StartsWith("http://", StringComparison.OrdinalIgnoreCase)
            ...
            ? relativePath
            : $"{baseUrl}/api/data/v9.0/{relativePath}";
        ...
        string escapedTable)
    {
        ...
        // Add collection properties in Studio's expected order:
        // Privileges first, then Attributes and relationships.
    ...
    /// <summary>
    /// Normalizes entity metadata to match the format Power Apps Studio produces.
    /// Rewrites the <c>@odata.context</c> URL and reorders JSON properties so
    /// that <c>Privileges</c> appears before <c>Attributes</c> and relationship
    /// collections, matching Studio's expected ordering.
    /// </summary>
    private static void NormalizeEntityMetadataContextUrl(JsonObject entityMetadata, string environmentUrl)
    {
        var existing = entityMetadata["@odata.context"]?.GetValue<string>();
        if (string.IsNullOrEmpty(existing))
        {
            return;
        }

        // Extract base URL up to $metadata# prefix
        var metadataIdx = existing.IndexOf("$metadata#", StringComparison.Ordinal);
        if (metadataIdx < 0)
        {
            return;
        }

        var baseUrl = existing.Substring(0, metadataIdx + "$metadata#".Length);
        entityMetadata["@odata.context"] = baseUrl
            + "EntityDefinitions(MetadataId,LogicalName,DisplayCollectionName,EntitySetName,Description,HasNotes,IsActivity,IsIntersect,IsManaged,TableType,IsPrivate,IsLogicalEntity,PrimaryIdAttribute,PrimaryNameAttribute,ObjectTypeCode,OwnershipType,Privileges,IsAvailableOffline,IsOfflineInMobileClient,Attributes(),ManyToOneRelationships(),OneToManyRelationships(),ManyToManyRelationships())/$entity";

        // Studio expects Privileges before Attributes and relationship
        // collections. The OData API returns Attributes first (based on
        // $expand order), so reorder to match Studio's format.
        ReorderEntityMetadataCollections(entityMetadata);
    }

    /// <summary>
    /// Reorders the collection/navigation properties in entity metadata so that
    /// <c>Privileges</c> appears before <c>Attributes</c>,
    /// <c>ManyToOneRelationships</c>, <c>OneToManyRelationships</c>, and
    /// <c>ManyToManyRelationships</c>, matching the order Power Apps Studio
    /// produces.
    /// </summary>
    private static void ReorderEntityMetadataCollections(JsonObject entityMetadata)
    {
        // The target order for collection properties is:
        // ...scalar props..., Privileges, Attributes, ManyToOneRelationships,
        // OneToManyRelationships, ManyToManyRelationships
        var collectionKeys = new[]
        {
            "Privileges",
            "Attributes",
        ...
        // Extract collection values, removing them from the object
        var extracted = new List<(string Key, JsonNode? Value)>();
        foreach (var key in collectionKeys)
        {
            if (entityMetadata.ContainsKey(key))
            {
                var value = entityMetadata[key];
                entityMetadata.Remove(key);
                extracted.Add((key, value?.DeepClone()));
            }
        }

        // Re-add in the correct order (appended at end, after all scalar props)
    ...
    /// <summary>
    /// Builds the entity icon URL using the pattern Power Apps expects: {baseUrl}/_imgs/ico_16_{objectTypeCode}.gif.
    /// Falls back to empty string when ObjectTypeCode is not available.
    /// </summary>
    private static string BuildIconUrl(string normalizedEnvironmentUrl, JsonObject entityMetadata)
    {
        var objectTypeCode = entityMetadata["ObjectTypeCode"]?.GetValue<int?>() ?? null;
        if (objectTypeCode is null)
        {
            return string.Empty;
        }

        return $"{normalizedEnvironmentUrl.TrimEnd('/')}/_imgs/ico_16_{objectTypeCode}.gif";
    }
