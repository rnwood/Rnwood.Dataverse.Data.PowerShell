...
{    public sealed record EmbeddedTemplateSummary(string Name, string Version, string TemplateId, string YamlControlName, bool RequiresVariantKeyword, IReadOnlyList<string> AvailableVariants, IReadOnlyList<TemplateAppFlagRequirement> AppFlagRequirements);
    ...
    /// <summary>
    /// Packs a directory of unpacked .msapp content into a new .msapp file.
    /// </summary>
    /// <param name="unpackDirectory">Path to unpacked app directory.</param>
    /// <param name="outputMsappPath">Output .msapp file path.</param>
    /// <param name="ignoreMissingDataSources">When true, ignore missing data source errors.</param>
    /// <param name="templateSourceMsappPaths">Optional extra .msapp files to use as control template sources.
    /// Templates from these files take priority over the built-in default templates. When multiple sources
    /// contain the same template, the latest version wins.</param>
    /// <param name="suppressDefaultTemplates">When true, do not include the built-in AllControls.msapp
    /// template library. Only the app's own templates and those from <paramref name="templateSourceMsappPaths"/>
    /// will be used.</param>
    public static void PackFromDirectory(
        string unpackDirectory,
        string outputMsappPath,
        bool ignoreMissingDataSources = false,
        ...
        bool suppressDefaultTemplates = false)
    {
        ...
        // Build the reference entries list: extra sources first (highest priority), then embedded AllControls (if not suppressed).
        var referenceEntriesList = BuildReferenceEntriesList(templateSourceMsappPaths, suppressDefaultTemplates);

        var deserializer = new DeserializerBuilder().IgnoreUnmatchedProperties().Build();
        var serializer = new SerializerBuilder().Build();
        var templateResolver = BuildTemplateResolver(entries, unpackDirectory, referenceEntriesList);
        var metadataResolver = BuildControlMetadataResolver(entries, referenceEntriesList,
            userProvidedReferenceStartIndex: suppressDefaultTemplates ? 0 : 1);
        var propertyResolver = BuildTemplatePropertyResolver(entries, referenceEntriesList);
        var keywordResolver = BuildTemplateKeywordResolver(entries, referenceEntriesList);
        var appFlags = ReadAppPreviewFlagsMap(entries);

        // Auto-enable classiccontrols when any YAML source file explicitly uses the Classic/ prefix.
        // This mirrors Power Apps Studio, which automatically sets classiccontrols=true whenever a
        // classic control is present in the app.
        if (!appFlags.TryGetValue("classiccontrols", out _))
        {
            var srcYamlEntries = entries
                .Where(e => e.Key.StartsWith("Src/", StringComparison.OrdinalIgnoreCase)
                            && e.Key.EndsWith(".pa.yaml", StringComparison.OrdinalIgnoreCase));
            var hasClassicControl = srcYamlEntries.Any(e =>
                Encoding.UTF8.GetString(e.Value).Contains("Classic/", StringComparison.Ordinal));

            if (hasClassicControl)
            {
                appFlags = AutoInjectAppFlag(entries, appFlags, "classiccontrols", true);
            }
        }

        var flagRequirementsMap = BuildTemplateFlagRequirementsMap(referenceEntriesList);
        ...
        UpdateUsedTemplates(entries, topControlFiles, topComponentFiles, referenceEntriesList);
