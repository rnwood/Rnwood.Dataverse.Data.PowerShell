...
{    private static void NormalizeThemeMetadata(Dictionary<string, byte[]> entries)
    ...
    private static bool NormalizeAppInfoStateAndRuleOrder(JsonObject appNode)
    {
        ...
        {
            "MinScreenHeight", "MinScreenWidth", "ConfirmExit", "SizeBreakpoints", "BackEnabled", "Theme",
        };
        ...
        foreach (var property in preferredStateOrder)
        {
            ...
            {
                stateNode = JsonValue.Create(property)!;
            }

            reorderedState.Add(stateNode.DeepClone());
        }

        // Append any state entries that exist but aren't in preferredStateOrder (e.g. user-defined OnStart/StartScreen)
        var preferredSet = new HashSet<string>(preferredStateOrder, StringComparer.OrdinalIgnoreCase);
        foreach (var (property, stateNode) in stateByProperty)
        {
            if (!preferredSet.Contains(property))
            {
                reorderedState.Add(stateNode.DeepClone());
            }
        }
    ...
    {
        JsonObject root;
        var hadExistingProperties = false;
        if (entries.TryGetValue("Properties.json", out var bytes)
            && JsonNode.Parse(bytes) is JsonObject parsed)
        {
            root = parsed;
            hadExistingProperties = true;
        }
        else
        {
            root = new JsonObject();
        ...
        // Always ensure required fields exist. The Ensure* helpers never overwrite
        // existing values, so this is safe for roundtrip of fully-populated apps AND
        // fills in missing fields for apps created via InitEmptyAppDirectory.
        {
            ...
            if (!hadExistingProperties) EnsureString(root, "AppCopilotSchemaName", "");
            ...
            if (!hadExistingProperties) EnsureDouble(root, "DeserializationLoadTime", 0.0);
        ...
        var newPropertiesJson = root.ToJsonString(JsonOptions);
        if (hadExistingProperties)
        {
            // If normalization made no semantic changes, preserve original byte formatting
            var originalCanonical = JsonNode.Parse(bytes)?.ToJsonString(JsonOptions);
            if (originalCanonical == newPropertiesJson)
            {
                return;
            }
        }

        entries["Properties.json"] = Encoding.UTF8.GetBytes(newPropertiesJson);
    }
    ...
    {
        JsonObject root;
        var hadExistingHeader = false;
        if (entries.TryGetValue("Header.json", out var bytes)
            && JsonNode.Parse(bytes) is JsonObject parsed)
        {
            root = parsed;
            hadExistingHeader = true;
        }
        else
        {
            root = new JsonObject();
        ...
        // Fill in any required version fields that are missing; Power Apps Studio refuses
        // to import a package with an empty or version-less Header.json.
        // We always check individual fields rather than early-returning for existing headers,
        // because InitEmptyAppDirectory creates a minimal "{}" Header.json that needs filling.
        ...
        var newHeaderJson = root.ToJsonString(JsonOptions);
        if (hadExistingHeader)
        {
            // If normalization made no semantic changes, preserve original byte formatting
            var originalCanonical = JsonNode.Parse(bytes)?.ToJsonString(JsonOptions);
            if (originalCanonical == newHeaderJson)
            {
                return;
            }
        }

        entries["Header.json"] = Encoding.UTF8.GetBytes(newHeaderJson);
    }
