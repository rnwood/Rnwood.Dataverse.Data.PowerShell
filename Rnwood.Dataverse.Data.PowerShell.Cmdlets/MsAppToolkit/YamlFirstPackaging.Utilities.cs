...
{
    /// <summary>
    /// Builds the ordered list of reference entry sets for template resolution.
    /// The embedded AllControls.msapp is loaded first (lowest priority) so that
    /// extra .msapp sources loaded afterwards can override its control prototypes.
    /// This ordering matters because <c>RegisterFromControlNode</c> uses last-write-wins
    /// semantics (<c>MergeRulesWithSourcePriority</c>), so later sources have higher
    /// priority for rule default values.
    /// </summary>
    private static IReadOnlyList<Dictionary<string, byte[]>> BuildReferenceEntriesList(
        IReadOnlyList<string>? templateSourceMsappPaths,
        bool suppressDefaultTemplates)
    {
        var list = new List<Dictionary<string, byte[]>>();

        // Embedded AllControls goes first — lowest priority (last-write-wins in prototype merge).
        if (!suppressDefaultTemplates)
        {
            ReadEmbeddedAllControlsEntries(out var embeddedEntries);
            list.Add(embeddedEntries);
        }

        // Extra template sources go last — highest priority.
        if (templateSourceMsappPaths is not null)
        {
            foreach (var path in templateSourceMsappPaths)
            {
                if (!string.IsNullOrWhiteSpace(path))
                {
                    list.Add(ReadEntriesFromMsApp(path));
                }
            }
        }
    ...
    private static JsonObject CreateDefaultApp(string startScreen)
    {
        ...
        app["ControlPropertyState"] = new JsonArray("ConfirmExit", "BackEnabled", "MinScreenHeight", "MinScreenWidth", "Theme", "SizeBreakpoints");
