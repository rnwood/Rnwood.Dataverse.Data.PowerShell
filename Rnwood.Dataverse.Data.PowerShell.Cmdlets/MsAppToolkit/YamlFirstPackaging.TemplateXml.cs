...
{    private static IReadOnlyList<UsedTemplate> ReadUsedTemplates(IReadOnlyDictionary<string, byte[]> entries)
    ...
    private static IReadOnlyList<TemplateAppFlagRequirement> BuildAppFlagRequirements(string templateXml)
    {
            ...
            && !categories.Contains("Display", StringComparer.OrdinalIgnoreCase))
        {
            AddUnique(new TemplateAppFlagRequirement(
                ...
                "Classic Control (Input) -- when classiccontrols=false (the default)."));
        }
    ...
    /// <summary>
    /// Injects a flag value into both the in-memory entries (Properties.json) and the returned
    /// flags dictionary.  Call this before property/flag validation to propagate an automatically
    /// inferred flag (e.g. classiccontrols=true when Classic/ controls are present in source YAML).
    /// </summary>
    private static IReadOnlyDictionary<string, bool> AutoInjectAppFlag(
        Dictionary<string, byte[]> entries,
        IReadOnlyDictionary<string, bool> currentFlags,
        string flagName,
        bool flagValue)
    {
        // Build a mutable copy of the flags dictionary with the new value.
        // Use ToDictionary for net462 compatibility (no Dictionary ctor accepting IReadOnlyDictionary + IEqualityComparer).
        var updated = currentFlags.ToDictionary(kvp => kvp.Key, kvp => kvp.Value, StringComparer.OrdinalIgnoreCase);
        updated[flagName] = flagValue;

        // Persist the change into Properties.json so it ends up in the packed msapp.
        try
        {
            var root = entries.TryGetValue("Properties.json", out var bytes)
                ? (JsonNode.Parse(Encoding.UTF8.GetString(bytes))?.AsObject() ?? new JsonObject())
                : new JsonObject();

            if (root["AppPreviewFlagsMap"] is not JsonObject flagsNode)
            {
                flagsNode = new JsonObject();
                root["AppPreviewFlagsMap"] = flagsNode;
            }
        ...
        {
            // Ignore serialization failures — the in-memory flags dict is still updated.
        }
    ...
    /// derived from the embedded All controls.msapp template XMLs and optional extra template sources.
    /// </summary>
    private static IReadOnlyDictionary<string, IReadOnlyList<TemplateAppFlagRequirement>> BuildTemplateFlagRequirementsMap(
        IReadOnlyList<Dictionary<string, byte[]>>? referenceEntriesList = null)
    {
        var result = new Dictionary<string, IReadOnlyList<TemplateAppFlagRequirement>>(StringComparer.OrdinalIgnoreCase);

        IEnumerable<Dictionary<string, byte[]>> sources;
        if (referenceEntriesList is not null && referenceEntriesList.Count > 0)
        {
            sources = referenceEntriesList;
        }
        else
        {
            ReadEmbeddedAllControlsEntries(out var embeddedEntries);
            sources = new[] { embeddedEntries };
        }

        foreach (var refEntries in sources)
        {
            foreach (var t in ReadUsedTemplates(refEntries))
            {
                if (!result.ContainsKey(t.Name))
                {
                    result[t.Name] = BuildAppFlagRequirements(t.TemplateXml);
                }
            }
        }
        ...
        IReadOnlyDictionary<string, IReadOnlyList<TemplateAppFlagRequirement>> flagRequirementsMap)
    {
        ...
        foreach (var req in requirements)
        {
            ...
            {
                // Some first-party exported apps include controls whose template XML declares
                // flag requirements that are not reflected in AppPreviewFlagsMap at runtime.
                // Enforcing these as hard errors breaks unpack/pack roundtrips of valid apps.
                // Keep processing and preserve source fidelity.
                continue;
            }
        }
    }
