...
```
Grant-DataverseAccess -Connection <ServiceClient> [-Target <object>] [-Principal <object>] [-PrincipalTableName <string>] [-AccessRights <AccessRights>] [-WhatIf] [-Confirm] [<CommonParameters>]
```

## DESCRIPTION

This cmdlet wraps the `GrantAccessRequest` SDK message. It executes the operation through the Dataverse Organization Service.
...
```
### -AccessRights
Parameter for the GrantAccessRequest operation
...
Returns the response from the `GrantAccessRequest` operation.
