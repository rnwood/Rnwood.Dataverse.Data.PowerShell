...
using System.Text;
using ValueType = Rnwood.Dataverse.Data.PowerShell.Commands.ValueType;
...
namespace Rnwood.Dataverse.Data.PowerShell.FrameworkSpecific.Commands.Internal
{
    internal class DataverseEntityConverter
    {
        public DataverseEntityConverter(IOrganizationService service)
        {
            ...
            entityMetadataFactory = new EntityMetadataFactory(service);
        }
            ...
            switch (attributeMetadata.AttributeType.Value)
            {
                ...
                case AttributeTypeCode.State:
                    ...
                    if (optionSetValue != null)
                    {
                        ...
                        return option != null ? (object)option.Label.UserLocalizedLabel.Label : optionSetValue.Value;
                    }
        ...
        public object ConvertToDataverseValue(EntityMetadata entityMetadata, string propertyName, AttributeMetadata attributeMetata, object psValue, ConvertToDataverseEntityColumnOptions columnOptions)
        {
            ...
            switch (attributeMetata.AttributeType.Value)
            {
                case AttributeTypeCode.PartyList:
                    if (psValue is IEnumerable)
                    {
                ...
                case AttributeTypeCode.Uniqueidentifier:
                    {
                        if (!string.IsNullOrWhiteSpace(stringValue))
                        {
                            if (entityMetadata.IsIntersect.GetValueOrDefault())
                            {
                                ...
                                if (manyToManyRelationshipMetadata.Entity1IntersectAttribute.Equals(attributeMetata.LogicalName, StringComparison.OrdinalIgnoreCase))
                                {
                                    lookupEntity = manyToManyRelationshipMetadata.Entity1LogicalName;
                                }
                                else if (manyToManyRelationshipMetadata.Entity2IntersectAttribute.Equals(attributeMetata.LogicalName, StringComparison.OrdinalIgnoreCase))
                                {
                ...
                case AttributeTypeCode.State:
                    {
                        if (!string.IsNullOrWhiteSpace(stringValue))
                        {
                            EnumAttributeMetadata pickListAttributeMetadata = (EnumAttributeMetadata)attributeMetata;
                ...
                case AttributeTypeCode.Customer:
                    {
                        ...
                        {
                            LookupAttributeMetadata lookupAttributeMetadata = (LookupAttributeMetadata)attributeMetata;
                            convertedValue = null;
                            ...
                            if (lookupAttributeMetadata.Targets == null || lookupAttributeMetadata.Targets.Length == 0)
                            {
                                ...
                                lookupAttributeMetadata.Targets = entityMetadata.ManyToOneRelationships.Where(r => r.ReferencingAttribute.Equals(attributeMetata.LogicalName, StringComparison.OrdinalIgnoreCase)).Select(r => r.ReferencedEntity).ToArray();
                            }

                            if (lookupAttributeMetadata.Targets.Length == 0)
                            {
                                throw new FormatException("Could not determine the target tables for lookup column " + attributeMetata.LogicalName + " - Dataverse metadata returned 0 targets!");
                            }
                ...
                default:
                    throw new NotImplementedException("Conversion to column type " + attributeMetata.AttributeType.Value + " not implemented");
            }
