    ...
    It "Can create, read, update, and delete all attribute types comprehensively" {
        pwsh -noninteractive -noprofile -command {
            ...
            try {
                ...
                $existingEntities = Get-DataverseEntityMetadata | Where-Object { $_.LogicalName -like "new_e2eattr_*" }
                ...
                Set-DataverseAttributeMetadata `
                    ...
                    -MaxLength 2000 `
                    -Confirm:$false
                Write-Host "✓ Memo attribute created"
                
                # Publish all changes
                Write-Host "Publishing all attributes..."
                Invoke-DataverseRequest -Connection $connection -Request @{
                    '@odata.type' = 'Microsoft.Crm.Sdk.Messages.PublishXmlRequest'
                    ParameterXml = "<importexportxml><entities><entity>$entityName</entity></entities></importexportxml>"
                }
                Write-Host "✓ Attributes published"
                
                Write-Host "Step 10: Reading and verifying all attributes..."
                $attributes = Get-DataverseAttributeMetadata -EntityName $entityName
                ...
                $updatedEmail = Get-DataverseAttributeMetadata -EntityName $entityName -AttributeName "new_email"
                ...
                $updatedLookup = Get-DataverseAttributeMetadata -EntityName $entityName -AttributeName "new_accountid"
                ...
                Remove-DataverseAttributeMetadata -EntityName $entityName -AttributeName "new_notes" -Confirm:$false
                ...
                $remainingAttrs = Get-DataverseAttributeMetadata -EntityName $entityName | Where-Object { $_.LogicalName -eq 'new_notes' }
                if ($remainingAttrs) {
