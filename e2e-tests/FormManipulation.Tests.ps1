    ...
    It "Comprehensively exercises all form manipulation features with cleanup" {
        pwsh -noninteractive -noprofile -command {
            ...
            try {
                ...
                # Test -Hidden parameter to verify cell-level visibility attribute
                Write-Host "  Testing -Hidden parameter (for bug fix validation)..."
                ...
                Write-Host "  Updated control 'primarycontactid' to hidden"
                
                # Verify the Hidden property is set correctly
                $hiddenControl = Invoke-WithRetry {
                    Wait-DataversePublish -Connection $connection -Verbose
                    Get-DataverseFormControl -Connection $connection `
                    -FormId $formId `
                    -TabName "CustomTab" `
                ...
                if (-not $hiddenControl.Hidden) {
                    throw "Control 'primarycontactid' should be hidden but Hidden property is: $($hiddenControl.Hidden)"
                }
                
                Write-Host "  Verified: Control 'primarycontactid' is hidden (Hidden=$($hiddenControl.Hidden))"
